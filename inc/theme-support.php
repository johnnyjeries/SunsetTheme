...
function sunset_register_nav_menu(){
    /*first param is the location of the menu, we can call it anything, and then we call it from the frontend,
    2nd param is the description, it is optional but it is good to explain for the user which type of menu is this  */
...
// custom function to print whatever we want inside our blog loop
function sunset_posted_meta(){
    //if we use return, then to use the function we have to echo it
    // human_time_diff() checks the time diff between 2 different values of time
    //get_the_time of the blogpost, U is the in a time stamp format
    ...
    if( !empty($categories) ):
        foreach( $categories as $category ):
            ...
            . esc_attr( 'View all posts in%s', $category->name ) .'">'. esc_html( $category->name ) .'</a>';
        $i++; endforeach;
    endif;
...
function sunset_posted_footer(){
    ...
    . get_the_tag_list( '<div class="tags-list"><span class="glyphicon glyphicon-tags"></span>',' ', '</div>' )
...
function sunset_get_embedded_media( $type = array() ){
    /* do_shortcode() gives us access to use a filter or a shortcode from wordpress
    we are forcing the generation of the code without printing it, we are storing it inside a variable */
    $content = do_shortcode( apply_filters( 'the_content', get_the_content() ) );
    //specifying which type of media we are accessing
...
function sunset_share_this( $content ){
	...
	if( is_single() ){
		...
		$content .= '<ul>';
		$content .= '<li><a href="' . $twitter . '" target="_blank" rel="nofollow"><span class="sunset-icon sunset-twitter"><img src="' . home_url('/wp-content/uploads/2023/07/icons8-twitter-30.png') . '"></span></a></li>';
        $content .= '<li><a href="' . $facebook . '" target="_blank" rel="nofollow"><span class="sunset-icon sunset-facebook"><img src="' . home_url('/wp-content/uploads/2023/07/icons8-facebook-30.png') . '"></span></a></li>';
		$content .= '<li><a href="' . $google . '" target="_blank" rel="nofollow"><span class="sunset-icon sunset-googleplus"><img src="' . home_url('/wp-content/uploads/2023/07/icons8-google-plus-50.png') . '"></span></a></li>';
